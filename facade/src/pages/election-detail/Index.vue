<template>
	<q-page padding>
		<q-breadcrumbs>
			...
			<q-breadcrumbs-el label="Election" icon="how_to_vote" />
		</q-breadcrumbs>
		<p class="text-h4 q-my-md">CANDIDATE</p>
		<q-table id="main-table" :rows="table.rows" :columns="table.columns" :filter="table.filter"
			:pagination="table.paginationSettings" :loading="table.loading" row-key="id"
			:visibleColumns="fullColumns">
			<template v-slot:top>
				<div>
					<q-btn class="q-mb-xs q-mr-xs" @click="onStore()" color="blue-grey-8" icon="add" size="md"
						unelevated>
						<template v-slot:default>
							<span class="q-mx-xs gt-xs">REGISTER CANDIDATE</span>
						</template>
					</q-btn>
				</div>
				<q-space />
			...
			<template v-slot:body="props">
				<q-tr class="table-row-height" :props="props">
					...
					<q-td key="candidate" :props="props"><span @click="onUpdate(props.row)" class="table-primary-field">
                        {{ props.row.candidate }}
                    </span></q-td>
		...
		<StoreOrUpdateDialog :row="dialog.row" :showDialog="dialog.showStoreOrUpdate" :candidateCache="dialog.candidateOptions" :positionCache="dialog.positionOptions"/>
	</q-page>
</template>
...
export default {
	...
	data() {
		return {
			...
			fullColumns: ['number', 'candidate', 'position'],
			table: {
				columns: [
					{ name: "number", class: "text-weight-bold", label: "#", align: "left" },
					{ name: "candidate", align: "left", label: "Candidate", field: "candidate", sortable: true },
	...
	methods: {
		...
		executeLoadTableRows() {
			...
			this.table.rows = [];
			this.$http.get('api/election-detail')
				.then(response => {
					this.table.rows = response.data[0];
                    this.dialog.candidateOptions = response.data[1];
                    this.dialog.positionOptions = response.data[2];
