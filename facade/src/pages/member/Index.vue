<template>
    <q-page padding>
        <q-breadcrumbs>
            ...
            <q-breadcrumbs-el label="Positions" icon="person" />
        </q-breadcrumbs>
        <p class="text-h4 q-my-md">Members</p>
            ...
            <template v-slot:body="props">
                <q-tr class="table-row-height" :props="props">
                    ...
                    <q-td key="status" :props="props">
                        <span :class="props.row.status == 1 ? 'text-green' : 'text-grey'" >&#11044;</span>
                        {{ props.row.status == 1 ? 'Active' : 'Inactive'}}</q-td>
                    <q-td key="admin" :props="props">
                        <!-- <span :class="props.row.admin == 1 ? 'text-green' : 'text-grey'" >&#11044;</span> -->
                        {{ props.row.admin == 1 ? 'Admin' : 'Member'}}
                    </q-td>
                    <q-td key="candidate" :props="props">
                        <!-- <span :class="props.row.candidate == 1 ? 'text-green' : 'text-grey'" >&#11044;</span> -->
                        {{ props.row.candidate == 1 ? 'Yes' : 'No' }}
                    </q-td>
                </q-tr>
            </template>
...
export default {
    ...
    data() {
        return {
            ...
            fullColumns: ['number', 'name', 'status', 'admin', 'candidate'],
            table: {
                columns: [
                    { name: "number", class: "text-weight-bold", label: "#", align: "left" },
                    { name: "name", align: "left", label: "Name", field: "name", sortable: true },
                    { name: "status", align: "left", label: "Status", field: "status", sortable: true },
                    { name: "admin", align: "left", label: "Admin", field: "admin", sortable: true },
