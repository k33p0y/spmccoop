<template>
	<q-page padding>
		<q-breadcrumbs>
			...
			<q-breadcrumbs-el label="Positions" icon="account_circle" />
		</q-breadcrumbs>
		<p class="text-h4 q-my-md">Positions</p>
				...
				<div>
					<q-btn class="q-mb-xs q-mr-xs" @click="onStore()" color="blue-grey-8" icon="add" size="md"
						...
						<template v-slot:default>
							<span class="q-mx-xs gt-xs">Create Position</span>
						</template>
					</q-btn>
				</div>
				<q-space />
...
export default {
	...
	methods: {
		...
		executeLoadTableRows() {
			...
			this.table.rows = [];
			this.$http.get('api/election/position')
				.then(response => {
