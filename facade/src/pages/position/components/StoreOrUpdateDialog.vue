        ...
        <q-card class="position-store-or-update">
            ...
            <q-card-section>
                <q-form class="q-gutter-sm" autocomplete="off">
                    ...
                    <q-input v-model="form.count" color="teal-8" :error="errors.count" :error-message="errors.countMessage" hide-bottom-space input-class="count" label="Count" outlined>
                        <template v-slot:label>
                            Count
                            <span class="text-negative">*</span>
                        </template>
...
export default {
    ...
    methods: {
        ...
        executeUpdate() {
            this.errors = {};
            this.loading = true;
            this.$http.patch('api/election/position/update', this.params())
                .then(() => {
                    this.$parent.$parent.$parent.executeLoadTableRows()
                    this.closeDialog();
                    this.loading = false;
